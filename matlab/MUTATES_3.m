...
% and a uniform noise between [-0.5,0.5] is then added to them.
...
function population = MUTATES_3(chromosome_size, population_size, nb_mutations, population,topology,inputs,outputs,range)
...
parfor m=1:nb_mutations
    ...
    temp_chromosome(genes) = temp_chromosome(genes)+(rand(1,length(genes))-0.5);
